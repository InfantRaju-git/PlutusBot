...
def run_trade_bot(symbol):
    ...
    print("Security Extraction Job", schedule_time)
    ...
    for hour in range(start_time.hour, end_time.hour + 1):
        for minute in range(start_time.minute, 60, Global.SYMBOL_SETTINGS[symbol]["TRADE_TF"]):
            ...
            if start_time <= schedule_time < end_time:
                ...
                schedule.every().day.at(schedule_time.strftime("%H:%M:%S")).do(BotMethods.trade_symbol, symbol=symbol, isOptionChart=True).tag('trade')

    print("Exit Trade Job", schedule_time)
